...
function renderGallery() {
  ...
  // Prepend the Notion 책장 가져오기 카드
  const notionCard = document.createElement('div');
  notionCard.className = 'notion-import-card';
  notionCard.innerHTML = `
    <span class="add-book-icon" style="filter: grayscale(1); transition: filter 0.3s;">🔗</span>
  ...
  sortedBooks.forEach((book, i) => {
    ...
    const ratingBadge = book.rating
      ? `<div class="card-rating-badge">${'★'.repeat(book.rating)}</div>` : '';
  ...
  if (books.length === 0) {
    ...
    if (supabaseClient) {
      try {
        const { data: { session } } = await supabaseClient.auth.getSession();
        if (session?.user) {
          const demoWithUser = demo.map(b => ({ ...b, user_id: session.user.id }));
          await supabaseClient.from('books').insert(demoWithUser);
        }
      } catch(e) {
        console.error(e);
      }
...
async function startNotionImport() {
  const urlInput = document.getElementById('notion-url').value.trim();
  if (!urlInput) {
    toast('⚠️ 노션 책장 링크를 입력해주세요.');
    return;
  }

  // Extract 32-character hex ID
  const match = urlInput.match(/[0-9a-f]{32}/i);
  if (!match) {
    toast('⚠️ 올바른 노션 페이지 링크가 아닙니다. 링크 형식을 확인해주세요.');
    return;
  }
  ...
  const progressDiv = document.getElementById('notion-progress');
  const progressBar = document.getElementById('notion-progress-bar');
  const progressStatus = document.getElementById('notion-progress-status');
  ...
  try {
    ...
    if (!recordMap) {
      throw new Error('노션 페이지 데이터를 불러오지 못했습니다.');
    }

    const targetBlock = recordMap.block?.[pageId]?.value?.value;
    const nickname = targetBlock?.properties?.title?.[0]?.[0] || 'Unknown';
    progressStatus.textContent = `크루 "${nickname}" 확인 완료! 완독책장 데이터베이스 찾는 중...`;
    progressBar.style.width = '15%';

    // Map collection_id to view_id
    const colToView = {};
    ...
    if (bookshelveCollections.length === 0) {
      throw new Error('가져올 완독책장 데이터베이스를 찾지 못했습니다.');
    }

    progressStatus.textContent = `완독책장 ${bookshelveCollections.length}개 발견. 도서 크롤링 시작...`;
    progressBar.style.width = '20%';

    const allBooks = [];
    const stepWeight = 70 / bookshelveCollections.length;

    for (let i = 0; i < bookshelveCollections.length; i++) {
      const shelf = bookshelveCollections[i];
      progressStatus.textContent = `[${i + 1}/${bookshelveCollections.length}] "${shelf.name}" 읽는 중...`;
      progressBar.style.width = `${Math.round(20 + i * stepWeight)}%`;
      ...
      try {
        ...
        if (!shelfRecordMap || !shelfRecordMap.block) continue;
        ...
        pages.forEach(p => {
          ...
          // Relation check (pointing to the crew page UUID)
          ...
          // Fallback nickname select/text check
          ...
          if (isUserBook) {
            ...
            if (covVal && Array.isArray(covVal)) {
              ...
              if (fileUrl) cover = fileUrl;
              else cover = covVal.map(s => s[0]).join('').trim();
            }
            ...
            allBooks.push({
              ...
              sentence,
              cover,
              rating: 5, // default maximum rating
              keywords,
              scraps: [],
        ...
        console.warn(`Error querying shelf ${shelf.name}:`, err);
      }
    }
    ...
    // Insert to Supabase if client is active
    if (supabaseClient && currentUser) {
      const booksToInsert = allBooks.map(b => ({
        ...b,
        user_id: currentUser.id
      }));

      // Supabase supports upsert to overwrite existing duplicates if key matches
      const { error } = await supabaseClient
        .from('books')
        .upsert(booksToInsert, { onConflict: 'id' });

      if (error) {
        throw new Error(`Supabase 동기화 실패: ${error.message}`);
      }
    }

    // Sync to local memory state and LocalStorage
    // Filter out duplicates by ID
    const importedIds = new Set(allBooks.map(b => b.id));
    books = books.filter(b => !importedIds.has(b.id));
    books.push(...allBooks);
    saveData();
    ...
    progressStatus.textContent = `오류: ${err.message}`;
