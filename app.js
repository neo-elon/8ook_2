...
/* ==============================================
   IMPORT READING LOGS (NEO 131 BOOKS / CSV)
============================================== */
...
async function loadNeoBooks131(mode = 'merge') {
  const statusMsg = document.getElementById('import-status-msg');
  const showStatus = (text, isError = false) => {
    if (statusMsg) {
      ...
      statusMsg.style.border = `1px solid ${isError ? '#ef4444' : '#10b981'}`;
  ...
  try {
    ...
    try {
      localStorage.setItem(`rj_books_backup_${Date.now()}`, JSON.stringify(books));
    } catch (e) { }

    let targetBooks = [];
    if (mode === 'replace') {
      targetBooks = neoBooks;
    } else {
      // Merge mode: keep existing books, add neoBooks if title not already present
      const existingTitles = new Set(books.map(b => (b.title || '').trim().toLowerCase()));
      const newItems = neoBooks.filter(b => !existingTitles.has((b.title || '').trim().toLowerCase()));
      targetBooks = [...newItems, ...books];
    }

    // Sort by date descending (latest first)
    targetBooks.sort((a, b) => (b.date || '').localeCompare(a.date || ''));
    ...
    if (supabaseClient && user) {
      showStatus('Supabase 데이터베이스와 동기화 중...');
      const payload = targetBooks.map(b => {
        const item = { ...b, user_id: user.id };
        return sanitizeBookForSupabase(item);
      });
    ...
    console.error(err);
    showStatus(`가져오기 실패: ${err.message}`, true);
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length >= 2 && cols[1]) {
          ...
          const ratingStr = cols[4] || '';
          const starCount = (ratingStr.match(/★/g) || []).length;
          const pages = parseInt(cols[5], 10) || 0;
          const keywords = (cols[6] || '').split(/[,/·]/).map(s => s.trim()).filter(Boolean);
          const sentence = (cols[7] || '').trim();
          ...
          newBooks.push({
            ...
            cover: '',
            spineCover: '',
            scraps: sentence ? [{ id: uid(), text: sentence, memo: '감상평', at: new Date().toISOString() }] : [],
            created_at: new Date().toISOString()
          });
        }
      }
      ...
      if (newBooks.length > 0) {
        ...
        updateSidebar();
        renderGallery();
        toast(`${newBooks.length}권의 도서를 성공적으로 가져왔습니다.`);
        closeModal('import-modal');
      } else {
        toast('유효한 도서 데이터를 찾지 못했습니다.');
      }
    } catch (err) {
      console.error(err);
      toast('CSV 파싱 오류: ' + err.message);
    }
  };
  reader.readAsText(file, 'utf-8');
}

window.loadNeoBooks131 = loadNeoBooks131;
